//
//  GetFavoriteComicUseCaseTests.swift
//  UseCaseTests
//
...
final class GetFavoriteComicUseCaseTests: XCTestCase {
    func test_getComicWithSuccess() {
        ...
        useCase.invoke(byId: 0) { (result: LocalResult<Comic?>) in
            switch result {
        ...
        let comic = anyComic()
        localDS.perform(.getComic, withSuccess: comic, byIndex: 0)
    ...
    // Mark - Helpers
    private func makeSUT() -> (useCase: GetFavoriteComicUseCase, localDS: ComicsLocalDataSourceSpy) {
        ...
        let useCase = GetFavoriteComicUseCase(comicsRepository: repository)
